...
function removeCtx(data) {
    if (Array.isArray(data)) {
        return data.map(item => {
            ...
            const { context } = item, rest = __rest(item, ["context"]);
        ...
        const { context } = data, rest = __rest(data, ["context"]);
        Object.keys(rest).forEach(key => {
            rest[key] = removeCtx(rest[key]);
        });
        return rest;
...
function diff(preModels, nextModels, isMod = true) {
    ...
    const delModels = deletedEntities.map(model => (Object.assign(Object.assign({}, model), { details: [`${label} ${model.name} 已删除`] })));
    const newModels = newEntities.map(model => (Object.assign(Object.assign({}, model), { details: [`${label} ${model.name} 已新增`] })));
    const updateModels = [];
    modifiedEntities.forEach(tEntity => {
        ...
        if (isMod) {
            ...
            if (msgs.length) {
                updateModels.push(Object.assign(Object.assign({}, nextEntity), { details: msgs }));
            }
        }
        else {
